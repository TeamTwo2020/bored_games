class Room{
	constructor(canvas, room_array, room_row_index, room_col_index, wall_thickness, left_door, right_door, upper_door, lower_door, room_color){
        ...
        this.particle_list = [];
        this.wall_list = this.generateWalls(12, canvas);
    ...
    generateWalls(amount_of_walls, canvas){
	    function not_block_gate(x,y){
	        if(y>=425&&y<=540&&x<=85) return false;//avoid block left gate
            if(y>=425&&y<=540&&x>=1185) return false;// avoid block right gate
        ...
        function not_block_gate_vertical(x,y){
            if(y>=285&&y<=540&&x<=85) return false;//avoid block left gate
            if(y>=285&&y<=540&&x>=1290) return false;// avoid block right gate
            if(y<=80&&x>=680&&x<=795) return false;// avoid block top gate
            if(y>=675&&x>=680&&x<=795) return false;// avoid block low gate
            else return true;
        ...
        function not_block_entity(x,y){
            if(y>=165&&y<=250&&x<=750&&x>=525) return false;//avoid block henry
            if(y>=15&&y<=80&&x<=100) return false;//avoid block hero
        ...
        function not_block_entity_vertical(x,y){
            if(y<=250&&x<=750&&x>=665) return false;//avoid block henry
            if(x>=15&&x<=80&&y<=100) return false;//avoid block hero
